...
{
    size_t idx;
    // Generate randomness for removing nodes
    random_buffer((uint8_t*)a_hat_0, l*sizeof(R_t)); random_buffer((uint8_t*)a_hat_1, l*sizeof(R_t));
//    random_buffer((uint8_t*)a_hat_0, l); random_buffer((uint8_t*)a_hat_1, l);
    random_buffer((uint8_t*)b_hat_0, l*sizeof(R_t)); random_buffer((uint8_t*)b_hat_1, l*sizeof(R_t));
//    random_buffer((uint8_t*)b_hat_0, l); random_buffer((uint8_t*)b_hat_1, l);
    random_buffer((uint8_t*)c_hat_0, l*sizeof(R_t));
//    random_buffer((uint8_t*)c_hat_0, l);
#if defined(_OPENMP)
    #pragma omp parallel for
#endif
    for (idx=0; idx<l; idx++)
    {
        c_hat_1[idx] = (a_hat_0[idx] + a_hat_1[idx]) * (b_hat_0[idx] + b_hat_1[idx])  - c_hat_0[idx];
    }
...
void funshade_shareSS_batch(size_t l, const R_t v[], const R_t hat_ab[], R_t hat_de[])
{
    ...
    for (idx=0; idx<l; idx++)
    {
        hat_de[idx] = v[idx] - hat_ab[idx];
    }
}

void funshade_node2del_batch(size_t l, bool j, const R_t hat_d[], const R_t hat_e[],
              const R_t hat_a_j[], const R_t hat_b_j[], const R_t hat_c_j[], R_t z_hat_j[])
{
    ...
    for (idx=0; idx<l; idx++)
    {
        z_hat_j[idx] = j * (hat_d[idx] * hat_e[idx]) + (hat_d[idx] * hat_b_j[idx]) + (hat_e[idx] * hat_a_j[idx])
                         + hat_c_j[idx];
    }
        ...
        R_t a_hat_0[], R_t a_hat_1[], R_t b_hat_0[], R_t b_hat_1[])
{
    ...
    random_buffer((uint8_t*)a_hat_0, l*sizeof(R_t)); random_buffer((uint8_t*)a_hat_1, l*sizeof(R_t));
//    random_buffer((uint8_t*)a_hat_0, l); random_buffer((uint8_t*)a_hat_1, l);
    random_buffer((uint8_t*)b_hat_0, l*sizeof(R_t));
//    random_buffer((uint8_t*)b_hat_0, l); random_buffer((uint8_t*)b_hat_1, l);
#if defined(_OPENMP)
    #pragma omp parallel for
#endif
    for (idx=0; idx<l; idx++)
    {
        b_hat_1[idx] = (a_hat_0[idx] + a_hat_1[idx]) * (a_hat_0[idx] + a_hat_1[idx])  - b_hat_0[idx];
    }
...
void funshade_shareSquare_batch(size_t l, const R_t v[], const R_t hat_a[], R_t e[])
{
    ...
    for (idx=0; idx<l; idx++)
    {
        e[idx] = v[idx] - hat_a[idx];
    }
}

void funshade_square_batch(size_t l, bool j, const R_t e[],
              const R_t hat_a_j[], const R_t hat_b_j[], R_t z_hat_j[])
{
    ...
    for (idx=0; idx<l; idx++)
    {
        z_hat_j[idx] = j * (e[idx] * e[idx]) + hat_b_j[idx] + (2 * e[idx] * hat_a_j[idx]);
    }
