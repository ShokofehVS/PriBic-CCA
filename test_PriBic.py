...
# Missing value imputation suggested by Cheng and Church for Yeast dataset
missing       = np.where((data <= 0.0))
data[missing] = np.random.randint(low=1, high=800, size=len(missing[0]))

# Missing value imputation suggested by Cheng and Church for Human dataset
...
# Avoid overflow (n parameter in paper)
max_bit = 2 ** 5
...
# Accuracy measurements (prelic and liu wang)
